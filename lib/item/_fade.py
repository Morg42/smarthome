...
def fade(item, dest, step=1, delta=1, caller=None,
         stop_fade=None, continue_fade=None,
         instant_set=True, update=False):
    """
    ...
    if stop_fade and not isinstance(stop_fade, list):
        logger.warning(
            f"stop_fade parameter {stop_fade} for fader {item} has to be a list. Ignoring"
        )
        stop_fade = None
    if continue_fade and not isinstance(continue_fade, list):
        logger.warning(
            f"continue_fade parameter {continue_fade} for fader {item} has to be a list. Ignoring"
        )
        continue_fade = None
