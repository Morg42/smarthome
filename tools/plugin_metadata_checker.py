...
def get_plugintype(plgName):
    ...
    try:
        ...
        return 'None'
    if (
        (code.find('(SmartPlugin') == -1)
        and (code.find('( SmartPlugin') == -1)
        and (code.find('(MqttPlugin') == -1)
        and (code.find('( MqttPlugin') == -1)
    ):
        return 'Classic'
    return 'Smart'
...
def list_formatted(
    plugin, plgvers, plgstate, plgtype, plgGlobal, plgParams, plgAttr, plgFunc='', plgLog='', list_versions=False
):
    if plgstate == 'qa-passed':
        plgstate = 'qa-pass'
    if not list_versions:
        print(
            '{plugin:<14.14} {plgvers:<8.8} {plgstate:<7.7} {plgtype:<5.5} {plgGlobal:<7.7} {plgParams:<7.7} {plgAttr:<10.10} {plgFunc:<7.7} {plgLog:<7.7}'.format(
                plugin=plugin,
                plgvers=plgvers,
                plgstate=plgstate,
                plgtype=plgtype,
        ...
        print(
            '{plugin:<14.14} {plgvers:<8.8} {plgstate:<7.7} {plgtype:<7.7} {plgShngMin:<9.9} {plgShngMax:<9.9} {plgPyMin:<9.9} {plgPyMax:<9.9}'.format(
                plugin=plugin,
                plgvers=plgvers,
                plgstate=plgstate,
                plgtype=plgtype,
    ...
    for plg in sorted(plugins_local):
        ...
        if (
            ...
            or (
                ...
                and (
                    plgstate == 'INVALID'
                    or sectionPlg == MISSING_TEXT
                    or sectionParam == MISSING_TEXT
                    or sectionIAttr == MISSING_TEXT
                    or sectionFunc == MISSING_TEXT
                    or sectionLogics == MISSING_TEXT
                )
            )
            or (
                option == 'compl'
                and (
                    plgtype.lower() != 'classic'
                    and sectionPlg != MISSING_TEXT
                    and sectionParam != MISSING_TEXT
                    and sectionIAttr != MISSING_TEXT
                    and sectionFunc != MISSING_TEXT
                )
                and (sectionLogics != MISSING_TEXT)
            )
            or (option == 'inc_para' and sectionParam == MISSING_TEXT)
            or (option == 'inc_attr' and sectionIAttr == MISSING_TEXT)
        ):
            if not list_versions:
                if not header_displayed:
                    ...
                    list_formatted(
                        'Plugin', 'Version', 'State', 'Type', 'Info', 'Params', 'Attrib.', 'Funct.', 'Logics'
                    )
                    list_formatted(ul, ul, ul, ul, ul, ul, ul, ul, ul)
                    header_displayed = True
                list_formatted(
                    plg, version, plgstate, plgtype, sectionPlg, sectionParam, sectionIAttr, sectionFunc, sectionLogics
                )
            else:
                # ----
                if not header_displayed:
...
def test_description(section, par, par_dict):
    ...
    if de == '' and en == '':
        disp_error(
            'No description of the ' + section + par + ' is given',
            "Add the section 'description:' to the " + section + "'s section and fill the needed values",
        )
    else:
        if de == '':
            disp_warning(
                'No german description of the ' + section + par + ' is given',
                "Add 'de:' to the description section of the " + section + '',
            )
        if en == '':
            disp_warning(
                'No english description of the ' + section + par + ' is given',
...
def check_metadata(
    ...
    only_inc=False,
    list_classic=False,
    pluginsdirectory=None,
    suppress_summery=False,
):
    ...
    if len(plg) > maxlen:
        disp_warning(
            f'A plugin name should not be longer than {maxlen} characters.',
            f"The plugin name '{plg}' is {len(plg)} characters long",
        )
    if plg != plg.lower():
    ...
    if metadata.get('plugin', None) is None:
        disp_error(
            'No global metadata defined',
            "Make sure to create a section 'plugin' and fill it with the necessary entries",
            'Take a look at the plugin development documentation of SmartHomeNG',
        )
        return
    else:
        ...
        if metadata['plugin'].get('classname', None) is None:
            disp_error(
                'No classname for the plugin',
                "Add 'classname:' to the plugin section and set it to the name of the Python class that implements the plugin",
            )
    ...
    if plg_type != 'classic' and not list_classic:
        if metadata.get('plugin', None) is None:
            ...
            if doc_url is None:
                disp_error(
                    "The 'documentation' parameter in section 'plugin' is explicitly declared as None - That is not allowed",
            ...
            if (doc_url is not None) and (doc_url != ''):
                if doc_url.endswith(f'plugins/{plg}/user_doc.html'):
                    disp_hint(
                        "The 'documentation' parameter in section 'plugin' should not contain an url to the SmartHomeNG documentation (user_doc)",
                        "The 'documentation' parameter is optional and only to be used to link to additional documentation outside of SmartHomeNG",
                        'If there is no additional documentation, leave this parameter empty',
                    )
                elif doc_url.endswith(f'plugins_doc/config/{plg}.html'):
                    disp_warning(
                        "The 'documentation' parameter in section 'plugin' should not contain an url to the SmartHomeNG documentation (configuration)",
                        "The 'documentation' parameter is optional and only to be used to link to additional documentation outside of SmartHomeNG",
                        'If there is no additional documentation, leave this parameter empty',
                    )
            ...
            if metadata['plugin'].get('support', '') == '':
                disp_hint(
                    "The 'support' parameter is empty.",
                    'Is there no support thread for this plugin in the knx-user-forum?',
                    "Enter the url to the support thread in the 'support' parameter.",
                )

            if metadata['plugin'].get('multi_instance', None) is None:
                disp_warning(
                    'It is not documented if wether the plugin is multi-instance capable or not',
                    "Add 'multi_instance:' to the plugin section",
                )
            else:
            ...
            if metadata['plugin'].get('restartable', None) is None:
                disp_hint(
                    'It is not documented if wether the plugin is restartable or not [stop() and run()]',
                    "Add 'restartable:' to the plugin section",
                )
            else:
            ...
            if metadata['plugin'].get('sh_minversion', None) is None:
                disp_warning(
                    'No minimum version of the SmartHomeNG core given that is needed to run the plugin',
                    "Add 'sh_minversion:' to the plugin section",
                )
        ...
        if metadata.get('parameters', None) is None:
            disp_error(
                'No parameters defined in metadata',
                "When defining parameters, make sure that you define ALL parameters of the plugin, but dont define global parameters like 'instance'. If only a part of the parameters are defined in the metadata, the missing parameters won't be handed over to the plugin at runtime.",
                "If the plugin has no parameters, document this by writing 'parameters: NONE' to the metadata file.",
            )

        # Checking item attribute metadata
        if metadata.get('item_attributes', None) is None:
            disp_error(
                'No item attributes defined in metadata',
                "If the plugin defines no item attributes, document this by creating an empty section. Write 'item_attributes: NONE' to the metadata file.",
            )

        # Checking item struct definitions
        if metadata.get('item_structs', None) is None:
            disp_hint(
                'No item structures defined in metadata',
                "If the plugin defines no item structures, document this by creating an empty section. Write 'item_structs: NONE' to the metadata file.",
            )

        # Checking function metadata
        if metadata.get('plugin_functions', None) is None:
            disp_error(
                'No public functions of the plugin defined in metadata',
                "If the plugin defines no public functions, document this by creating an empty section. Write 'plugin_functions: NONE' to the metadata file.",
            )

        # Checking logic parameter metadata
        if metadata.get('logic_parameters', None) is None:
            disp_error(
                'No logic parameters defined in metadata',
                "If the plugin defines no logic parameters, document this by creating an empty section. Write 'logic_parameters: NONE' to the metadata file.",
            )
            ...
            if metadata.get('parameters', None) != 'NONE':
                for par in metadata.get('parameters', None):
                    if par.lower() in ['instance', 'webif_pagelength']:
                        disp_warning(
                            f"parameter '{par}' should not be defined in the plugin",
                            f"Parameter '{par}' is a globaly defined plugin parameter. You should not explicitly define it in your plugin '{plg}'",
                        )
                    else:
                        par_dict = metadata['parameters'][par]
                        if not is_dict(par_dict):
                            ...
                            if par_dict.get('mandatory', False) and par_dict.get('default', None) is not None:
                                disp_error(
                                    "parameter '{}': mandatory and default cannot be used together".format(par),
                                    'If mandatory and a default value are specified togeather, mandatory has no effect, since a value for the parameter is already specified (the default value).',
                                )
            ...
            if metadata.get('item_attributes', None) != 'NONE':
                for par in metadata.get('item_attributes', None):
                    ...
                    if not is_dict(par_dict):
                        ...
                        if par_dict.get('mandatory', False) and par_dict.get('default', None) is not None:
                            disp_error(
                                "item '{}': mandatory and default cannot be used together".format(par),
                                'If mandatory and a default value are specified togeather, mandatory has no effect, since a value for the parameter is already specified (the default value).',
                            )
                ...
                for func in metadata.get('plugin_functions', None):
                    ...
                    if func_dict.get('parameters', None) is not None:
                        for par in func_dict.get('parameters', None):
                            ...
                            test_description(
                                "parameter '" + par + "' of plugin function", func, par_dict.get('description', None)
                            )
        ...
        if check_quiet:
            if not suppress_summery:
                if not (only_inc) or (only_inc and (errors != 0 or warnings != 0 or hints != 0 or state == '-')):
                    if state == 'qa-passed':
                        state = 'qa-pass'
                    summary = '{:<8.8} {:<7.7} {:<5.5} {:<8.8} {:<7.7} {}'.format(
                        res, state, plg_type, str(errors), str(warnings), str(hints)
                    )
...
if __name__ == '__main__':
    ...
    group.add_argument(
        '-la', '--list_all', action='store_true', default=False, help='list plugin information of all plugins'
    )
    group.add_argument(
        '-lcl', '--list_classic', action='store_true', default=False, help='list plugin information of classic plugins'
    )
    group.add_argument(
        '-lsm', '--list_smart', action='store_true', default=False, help='list plugin information of smart plugins'
    )
    group.add_argument(
        '-lst',
        '--list_state',
        action='store_true',
        default=False,
        help='list plugin information grouped by plugin state',
    )
    group.add_argument(
        '-li',
        '--list_inc',
        action='store_true',
        default=False,
        help='list information of plugins with incomplete metadata',
    )
    group.add_argument(
        '-lc',
        '--list_compl',
        action='store_true',
        default=False,
        help='list information of plugins with complete metadata',
    )
    group.add_argument(
        '-lip', action='store_true', default=False, help='list info of plugins with incomplete parameter data'
    )
    group.add_argument(
        '-lia', action='store_true', default=False, help='list info of plugins with incomplete item attribute data'
    )
    ...
    group.add_argument(
        '-cl', '--check_list', action='store_true', default=False, help='check the metadata of all plugins'
    )
    group.add_argument(
        '-clc',
        '--check_clist',
        action='store_true',
        default=False,
        help='check the metadata of plugins with all metadata sections',
    )
    group.add_argument(
        '-cli',
        '--check_ilist',
        action='store_true',
        default=False,
        help='check the metadata of all plugins, list only incomplete plugins',
    )
    group.add_argument(
        '-v', '--list_versions', action='store_true', default=False, help='list versions instead of metadata checks'
    )
