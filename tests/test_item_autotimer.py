...
class RecordingScheduler:
    ...
    def add(self, name, obj=None, prio=3, cron=None, cycle=None,
            value=None, offset=None, next=None, items=None):
        self.calls.append({
            'action': 'add', 'name': name,
            'cron': cron, 'cycle': cycle, 'value': value, 'next': next,
        })
...
class TestTimerPublicMethod(_Base):
    ...
    def test_timer_value_passed_to_scheduler(self):
        ...
        self.assertTrue(any(
            isinstance(c.get('value'), dict) and c['value'].get('value') == 99
            for c in add_calls
        ))
