...
class MockMqttModule:
    ...
    def subscribe_topic(self, caller, topic, callback=None, qos=None,
                        payload_type=None, bool_values=None):
        self.subscribed.append((caller, topic))

    def unsubscribe_topic(self, caller, topic):
        self.unsubscribed.append((caller, topic))

    def publish_topic(self, caller, topic, payload, qos=None,
                      retain=False, bool_values=None):
...
class MqttPluginContractTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        mock_modules.get_module.side_effect = (
            lambda name: self.mock_mqtt if name == 'mqtt' else None
        )

        self._modules_patcher = patch(
            'lib.module.Modules.get_instance', return_value=mock_modules
        )
    ...
    def test_inherits_from_mqttplugin(self):
        ...
        self.assertTrue(
            issubclass(self.PLUGIN_CLASS, MqttPlugin),
            f'{self.PLUGIN_CLASS.__name__} does not inherit from MqttPlugin'
        )
    ...
    def test_mod_mqtt_set_after_init(self):
        ...
        self.assertIsNotNone(
            self.plugin.mod_mqtt,
            'mod_mqtt is None after __init__ — mqtt module mock not wired correctly'
        )
    ...
    def test_start_subscriptions_calls_subscribe_topic(self):
        ...
        self.assertTrue(
            len(self.mock_mqtt.subscribed) >= 1,
            'subscribe_topic was never called after start_subscriptions()'
        )
    ...
    def test_stop_subscriptions_calls_unsubscribe_topic(self):
        ...
        self.assertTrue(
            len(self.mock_mqtt.unsubscribed) >= 1,
            'unsubscribe_topic was never called after stop_subscriptions()'
        )
    ...
    def test_publish_topic_calls_mod_mqtt(self):
        ...
        self.assertTrue(
            any(t == 'test/out' for _, t, _ in self.mock_mqtt.published),
            'mod_mqtt.publish_topic was not called'
        )
