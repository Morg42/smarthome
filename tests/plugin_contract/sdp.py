...
from lib.model.sdp.globals import (
    ITEM_ATTR_COMMAND, ITEM_ATTR_READ, ITEM_ATTR_WRITE,
    CMD_ATTR_OPCODE, CMD_ATTR_READ, CMD_ATTR_WRITE, CMD_ATTR_ITEM_TYPE,
    CONN_NULL,
)

# Valid shng item types (subset; the important ones for SDP)
_VALID_ITEM_TYPES = frozenset({
    'bool', 'num', 'str', 'list', 'dict', 'foo',
    'scene',
})
...
class SdpPluginContractTest(unittest.TestCase):
    ...
    def test_all_commands_have_opcode_or_read_cmd(self):
        ...
        missing = [
            path for path, defn in flat.items()
            if CMD_ATTR_OPCODE not in defn and 'read_cmd' not in defn
        ]
        self.assertFalse(
            missing,
            'Commands without opcode or read_cmd:\n  ' + '\n  '.join(missing)
        )
    ...
    def test_command_item_types_are_valid(self):
        ...
        self.assertFalse(
            errors,
            'Invalid item_type values in commands:\n  ' + '\n  '.join(errors)
        )
        ...
        for i, attr_set in enumerate(self.SDP_ITEM_ATTR_SETS):
            with self.subTest(attr_set=attr_set):
                ...
                self.assertIn(
                    ...
                    f'SDP_ITEM_ATTR_SETS[{i}]={attr_set!r} did not register the item'
                )
