...
class BasePluginContractTest(unittest.TestCase):
    ...
    def test_plugin_version_format(self):
        ...
        self.assertRegex(v, r"^\d+\.\d+\.\d+", f"PLUGIN_VERSION {v!r} does not start with a x.y.z version number")
    ...
    def test_allow_multiinstance_is_defined(self):
        ...
        self.assertIn(ami, (True, False), f"ALLOW_MULTIINSTANCE is {ami!r}; must be True or False")
    ...
    def test_run_method_is_overridden(self):
        ...
        self.assertNotEqual(
            self.PLUGIN_CLASS.run, SmartPlugin.run, f"{self.PLUGIN_CLASS.__name__} does not override run()"
        )

    def test_stop_method_is_overridden(self):
        """stop() must be overridden — SmartPlugin.stop() raises NotImplementedError."""
        self.assertNotEqual(
            self.PLUGIN_CLASS.stop, SmartPlugin.stop, f"{self.PLUGIN_CLASS.__name__} does not override stop()"
        )
    ...
    def test_alive_false_after_init(self):
        ...
        self.assertFalse(self.plugin.alive, "alive must be False after __init__; set it to True only inside run()")
