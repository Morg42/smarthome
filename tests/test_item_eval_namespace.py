...
# register SmartHomeNG custom log levels expected by the logging framework
def _reg_levels():
    for lvl, name in [(31, 'NOTICE'), (13, 'DBGHIGH'), (12, 'DBGMED'),
                      (11, 'DBGLOW'), (9, 'DEVELOP')]:
        if not hasattr(logging.getLoggerClass(), name.lower()):
            def _make(l):
                def _m(self, msg, *a, **kw):
                    if self.isEnabledFor(l):
                        self._log(l, msg, a, **kw)
                return _m
...
class TestMakeEvalEnvKeys(_Base):
    ...
    def setUp(self):
        ...
        self.ns = _make_eval_env(self.item, value=7, caller='Test',
                                 source='src', dest='dst')
...
class TestRunEvalItemAccess(_Base):
    ...
    def test_item_is_the_evaluated_item(self):
        ...
        captured = []
        i = _item(self.sh, 'item_identity',
                  eval='captured.append(item) or 42')
...
class TestRunEvalItemsAccess(_Base):
    ...
    def test_items_return_item_lookup(self):
        ...
        src(77)
        tgt = _item(self.sh, 'lookup_tgt',
                    eval='items.return_item("lookup_src")()')
        self._run(tgt)
    ...
    def test_items_return_item_missing_returns_none(self):
        ...
        tgt = _item(self.sh, 'lookup_miss',
                    eval='0 if items.return_item("does.not.exist") is None else 1')
        self._run(tgt)
    ...
    def test_items_return_item_reflects_updated_value(self):
        ...
        src(10)
        tgt = _item(self.sh, 'live_tgt',
                    eval='items.return_item("live_src")()')
        self._run(tgt)
...
class TestOnChangeNamespace(_Base):
    ...
    def test_math_in_on_change(self):
        ...
        source = _item(self.sh, 'oc_math_src',
                       on_change='oc_math_tgt = math.floor(value)')
        source(3.9)
...
class TestEvalCompatShim(_Base):
    ...
    def test_succeeds_with_os_in_legacy_namespace(self):
        ...
        result = _eval_with_legacy_fallback('type(os).__name__', self.ns,
                                            self.item, 'test', exc)
    ...
    def test_returns_eval_failed_when_both_namespaces_fail(self):
        ...
        result = _eval_with_legacy_fallback('totally_unknown_xyz_abc + 1',
                                            self.ns, self.item, 'test', exc)
    ...
    def test_warning_names_missing_variable(self):
        ...
        with self.assertLogs('lib.item', level='WARNING') as cm:
            _eval_with_legacy_fallback("re.sub('x','y','xyz')", self.ns,
                                       self.item, 'test', exc)
        self.assertTrue(any("'re'" in line or "'re'" in line for line in cm.output),
                        msg="Expected \"'re'\" in warning output")
    ...
    def test_logs_warning_on_total_failure(self):
        ...
        with self.assertLogs('lib.item', level='WARNING') as cm:
            _eval_with_legacy_fallback('totally_unknown_xyz', self.ns,
                                       self.item, 'test', exc)
    ...
    def test_re_module_available_in_legacy(self):
        ...
        exc = NameError("name 're' is not defined")
        result = _eval_with_legacy_fallback("re.sub('a','b','cat')", self.ns,
                                            self.item, 'test', exc)
        self.assertIsNot(result, _EVAL_FAILED)
...
class TestRunEvalCompatIntegration(_Base):
    ...
    def test_os_name_in_eval_succeeds_via_fallback(self):
        ...
        item = _item(self.sh, 'compat_os', eval="type(os).__name__",
                     itype='str')
        with self.assertLogs('lib.item', level='WARNING') as cm:
    ...
    def test_re_in_on_change_succeeds_via_fallback(self):
        ...
        source = _item(self.sh, 'compat_oc_src',
                       on_change="compat_oc_tgt = re.sub('o','0','foo')")
        with self.assertLogs('lib.item', level='WARNING') as cm:
            source(1)
